import $ from 'jquery';
export default function (time, layers) {
	//各种定义和参数
	var grid = []; //定义经纬度网格数组
	var particles = []; //存放粒子数组
	var PARTICLE_MULTIPLIER = 0.8 / 100; //粒子数量参数，默认1/300，可以根据实际调
	var max_age = 80; //定义粒子的最大运动次数
	//没有风的情况
	var NULL_WIND_VECTOR = [NaN, NaN, null];
	var min_color = 0; // 风速为0使用的颜色
	var max_color = 10; // 风速最大使用的颜色
	var linewidth = 20; //定义粒子粗细
	var FRAME_RATE = 10, //定义每秒执行的次数
		FRAME_TIME = 500 / FRAME_RATE; // desired frames per second
	var galpha = 0.85; //定义透明度，透明度越大，尾巴越长
	//存放颜色的数组
	var colorScale = ["rgb(219,221,224)"];
	...
	//第一步生成经纬度网格///////////////////////////////////////////////////
	$.getJSON("您的风流场接口", data => {
					evolve();  
					$.getJSON(data.result[0].geoUrl, res => {
						if (layers) {
							setTimeout(function () {
								window.map._panes.overlayPane.removeChild(layers)
							}, 200)
						}
						buildGrid(res);
				...
				var start = Date.now();
				var bwidth = window.map.getSize().x,
					bheight = window.map.getSize().y;
				var bx = 0,
					by = 0;
				var projection = {};
				var mbounds = window.map.getBounds();
				var now_bounds = {
				...
				var animated = window.map.options.zoomAnimation && L.Browser.any3d;
				//添加下面的class之后，图层可以随着地图缩放变化
				L.DomUtil.addClass(canvas, 'leaflet-zoom-' + (animated ? 'animated' : 'hide'));
				//把画布添加到overlayPane图层中
				window.map._panes.overlayPane.appendChild(canvas);
				var fadeFillStyle = "rgba(0, 0, 0, 0.97)";
				var g = canvas.getContext("2d");
				g.lineWidth = linewidth;
				g.fillStyle = fadeFillStyle;
				g.globalAlpha = 0.6;
				window.map.on("zoomanim", function (e) {
					var scale = window.map.getZoomScale(e.zoom);
					...
					var new_position = window.map.latLngToLayerPoint([yun_lat1, yun_lon2]);
					// -- different calc of offset in leaflet 1.0.0 and 0.0.7 thanks for 1.0.0-rc2 calc @jduggan1
					var offset = L.Layer ? window.map._latLngToNewLayerPoint(window.map.getBounds().getNorthWest(), e.zoom, e.center) : window.map.getCenterOffset(e.center).multiplyBy(-scale).subtract(window.map.getMapPanePos());

					L.DomUtil.setTransform(canvas, offset, scale);
				})
				window.map.on("moveend", function () {
					window.cancelAnimationFrame(animationLoop);
					bwidth = window.map.getSize().x,
						bheight = window.map.getSize().y;
